...
**Serialization claims must be audited against every await.** Any doc claim of the form "main-actor serialisation makes overlap impossible" (or any "actor isolation guarantees ordering" variant) must be checked against EVERY `await` in the method: each suspension point is a reentrancy window where another call can enter mid-method (SE-0306 — isolation serialises *between* suspension points, not across whole calls). If the method has an await, overlap is possible; document the gap or guard it (single-flight slot, in-flight flag, queue) — never assert it away. A synchronous test fake has no suspension point and therefore CANNOT exercise reentrancy; a real violation test needs a suspension-gated fake (see `SuspendingPermissionService` in `AudioCaptureViewModelTests`). Rationale: the 2026-06-10 mic-fix gate "fix" (`3d41090`) re-asserted "two main-actor calls cannot interleave" over a method whose await suspends for as long as the system permission dialog is on screen — the exact false-confidence failure this section exists to prevent, shipped by the very edit that claimed to cure it.
