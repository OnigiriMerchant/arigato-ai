...
- **HEAD = `c7577a2`** (Phase 7 onboarding entrance animation). App code HAS changed since `eea5abc`: the Phase 7 UI-polish chain `2c501b5`→`c7577a2` (12 commits — design pre-flight + reconcile, tokens, detail + live-view restyles, onboarding hero). All pushed; the state-sync docs refresh commits on top (push separately gated).
