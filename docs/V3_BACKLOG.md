...
- **Phase 7 reconciliation — LOCKED (2026-05-31).** Design decisions locked and the doc-vs-code drift the pre-flight found is being reconciled. Basis: `docs/PHASE_7_DESIGN_RESEARCH.md` (collisions A–E, commit `ae2cb15`). Several of the original "design opportunities" below are superseded; the locked direction is:
  ...
  - **Subagent question (posed below) — RESOLVED + DONE:** EXTEND `@ui-reviewer`'s mandate to own the design language; do **NOT** create a new `@design-system` subagent. The deferred items — the `@ui-reviewer` mandate rewrite **and** the CLAUDE.md "Design language (Phase 7)" rules — were authored against the now-shipped **real** token names (tokens shipped `9763aa1`) in the Phase 7 enforcement-encoding step. `@ui-reviewer` now carries the design-language ownership note + token-level enforcement checks; CLAUDE.md carries the rules summary. This closes the prior deferral.
  ...
  - **Dispatch chain (all shipped):** design-language reconciliation (`e337bf6`) → design tokens (`9763aa1`) → detail-view source-led restyle (`12709b5`) → checkpoint-2 system-automatic glass closure (`adfc99b`) → this enforcement-encoding step (CLAUDE.md rules + `@ui-reviewer` mandate).
...
- **Update (2026-05-31, post-MVP-1 — now fires on EVERY default combined run):** the "sprint-window" framing is stale (MVP-1 is feature-complete, no active sprint), but this flake is now MORE visible, not less. The Part-2 XCUITest QA harness landed this session (Groups A–D, `f7e9646`→`0a1c24e`), adding an `ArigatoAIUITests` target to the default `mcp__XcodeBuildMCP__test_sim` run. That UI-test target boots the simulator and drives the app — exactly the "issue 2" `FBSOpenApplicationServiceErrorDomain` boot-race load — so the **combined** unit + XCUITest run now reads **409 passed / 6 failed / 2 skipped** (417 total) where the unit suite alone read green. Confirmed flakes (not regressions): an isolation re-run the same night passed **14 / 14, 0 failed** (`-only-testing:ArigatoAITests/AppBootstrapperTests -only-testing:ArigatoAITests/AppBootstrapperMeetingWiringTests`, 30s). The 6 (file:line + failure shape):
  - `AppBootstrapperMeetingWiringTests.swift:285` `meetingStoreWrites_doNotBlockMainThread_under100SentenceBurst` — heartbeat yields 46, expected ≥100 (main-thread-blocking heuristic trips under load).
  - `AppBootstrapperMeetingWiringTests.swift:362` `startPrewarm_whenContainerIsNil_neverConstructsCoordinator` — LFM2 stuck `.loaded`, expected `.ready`.
  - `AppBootstrapperMeetingWiringTests.swift:509` `debugPublishMeetingStore_onWarmupFailure_publishesStoreOnly` — LFM2 stuck `.loaded`, expected `.failed` (test added with the DEBUG debug-seed-reachability chain).
  - `AppBootstrapperTests.swift:400` `T6.1 startPrewarm runs LFM2 after Whisper reaches .loaded` — LFM2 stuck `.loading`, expected `.ready`.
  - `AppBootstrapperTests.swift:481` `T6.4 startPrewarm LFM2 warmup failure advances to .failed(.warmupFailed)` — LFM2 stuck `.warming`, expected `.failed`.
  - `AppBootstrapperTests.swift:502` `T6.5 startPrewarm LFM2 happy path advances to .ready` — LFM2 stuck `.warming`, expected `.ready`.
  None of this session's production diffs (value-based `AppRoute` navigation, XCUITest harness, accessibility IDs, `confirmationDialog` fix) touches `AppBootstrapper` or the loader state machine, so these are load-induced, not behavioral. **User direction (2026-05-31): stabilize or quarantine so the combined suite reads deterministically green** — not blocking. See new fix option 5 below.
...
- **Trigger to fix:**
  ...
  - **Soft:** next test-infra hardening pass (the pre-MVP-1 sprint is closed) — i.e. whenever the non-deterministic combined-run signal becomes a dispatch-time annoyance. Apply fix option 5.
...
- **Fix options (in order of cost):**
  ...
  5. **Make the default combined run deterministically green (2026-05-31 user-requested direction).** Two complementary moves: **(a) Quarantine the UI tests** — split `ArigatoAIUITests` into its own test plan / scheme so the default `test_sim` runs the unit suite alone (deterministic green) and the XCUITest harness runs as a separate, explicitly-invoked job; this removes the simulator-boot-race load from the routine signal. **(b) Stabilize the prewarm assertions** — `.serialized` the two AppBootstrapper suites (option 1) AND replace the poll-until-state pattern with a deterministic state-transition handshake (await a continuation the loader fake fires on reaching `.ready`/`.failed`, instead of yielding/sleeping), and relax the 100-sentence heartbeat lower bound (or gate it behind a not-under-contention condition). (a) is the cheap "reads green now" lever; (b) is the real fix. ~30–60 min for (a); ~2–4h for (b). Either alone restores a trustworthy combined signal; do both for a deterministic suite that still exercises the UI flows.
...
- **Severity:** MED — standing combined-run determinism risk (no longer sprint-scoped). Does NOT block MVP-1 ship and is not a production bug, but as of 2026-05-31 it fires on **every** default `test_sim` run (the XCUITest harness made it routinely visible), so the default green signal can no longer be trusted without the isolation/quarantine workaround. Stabilize or quarantine per fix option 5 to restore a deterministic combined signal.
...
### LEAP SDK v0.10.x migration — ✅ DONE, adopted v0.10.9 (2026-06-06) — was: NOT adopting [2026-05-31], was: upstream-blocked (P-2 attempt closed)

- **STATUS (2026-06-06): ✅ DONE — MIGRATED to `leap-sdk` v0.10.9.** The 2026-05-31 "NOT adopting" decision below is **reversed and superseded**. The two legs the "not adopting" decision rested on are both now addressed: (1) the v0.10.9 @rpath fix is no longer merely release-notes-asserted — the device-reference build `~/AI-projects/arigato-ai-p2` is on 0.10.9 (rev `a83ca1e`), **compiles clean and deploys to a physical iPhone**, and this repo's app target compiles + links against the same revision; (2) the driver was channel consolidation onto the maintained `leap-sdk` repo + the bundled-GGUF load path, not a v0.10.x iOS feature. Code landed 2026-06-06 (revert tag `pre-leap-0.10.9-migration`): SPM `leap-ios`@0.9.4 → `leap-sdk.git`@0.10.9 (product `LeapModelDownloader`); bundled-GGUF `Leap.shared.load(url:)`; per-call `GenerationOptions().with(...)`; `LiquidCacheOptions.enabled(path:)`; `.error` (`MessageResponseError`, new in 0.10.9) → thrown stream finish; download plumbing removed (`LFM2LoaderState.downloading` kept-unreachable); nested-dylib re-sign Run Script phase (iphoneos-gated). Full record in `docs/CURRENT_STATE.md` "2026-06-06 session". **SKIE caveat:** verify leap-sdk APIs by COMPILING, not by grepping the `.swiftinterface` (SKIE overloads are generated outside it — false negatives). The historical "NOT adopting" / "upstream-blocked" notes below are retained as the pre-reversal audit trail.
- **STATUS (2026-05-31): RESOLVED upstream + decision locked NOT to adopt.** The @rpath launch crash is FIXED in `leap-sdk` **v0.10.9 (2026-05-29)**, per the maintainer's own release notes (attributed to ref #265), which explicitly resolve `dyld: Library not loaded: @rpath/inference_engine_llamacpp_backend.framework/…` affecting **v0.10.5–v0.10.8** — "the embedded `libinference_engine.dylib` now references the dylib that's actually shipped, so apps no longer crash at startup." **v0.10.x is no longer a dead end.** _Caveat:_ the fix is **release-notes-asserted, NOT binary- or device-verified by us** (no `otool -L` on v0.10.9's dylib has been run here). Issue #5 remains **OPEN** — that reflects the maintainer never closing a stale issue, **NOT** an unfixed bug; its 3 comments are all the reporter's own (OnigiriMerchant), dated 2026-05-20, none referencing v0.10.9. Verified 2026-05-31 against primary sources (leap-sdk releases/tag pages + the issue #5 GitHub API); see briefs #35/#36.
- **DECISION (locked 2026-05-31): NOT migrating to v0.10.9.** Rationale: none of v0.10.x's iOS features (image input, resize, Android/KMP infra) benefit a JA↔EN on-device meeting-translation app; **SDK version does not affect translation quality** — that is model-side (LFM2-350M-ENJP-MT), not SDK-side. MVP-1 ships on **v0.9.4**. **Standing principle:** upgrade the SDK only when a release delivers a benefit that materially touches THIS app, not for version currency.
- **Breaking Swift changes to budget IF adoption is ever triggered:** `LeapModelDownloader`→`ModelDownloader` rename + dynamic-framework packaging + dual-import guard (introduced v0.10.6); `LeapDownloaderConfig(…)`→`LeapDownloaderConfig.with(…)` static factory (#262, v0.10.9); image/audio factories now **throw** — `fromJPEGData` / `fromPNGData` / `fromImageData` / `fromRGB8` / `fromWAVData` / `fromFloatSamples` / `audio(data:format:)` (#264 / #267, v0.10.9). **Validate on a PHYSICAL iPhone** — community reports the v0.10.x crash surfaced in the simulator but on-device behavior differed; never trust a sim-only pass for this migration.
- **Historical block (now resolved — retained for context):** P-2 attempted v0.9.4 → v0.10.6 and was blocked because `libinference_engine.dylib` in v0.10.5–v0.10.8 recorded a `@rpath/inference_engine_llamacpp_backend.framework/inference_engine_llamacpp_backend` (framework-bundle form) dependency while the SDK shipped only `libinference_engine_llamacpp_backend.dylib` (plain dylib) → dyld launch crash before LFM2 reached `.ready`. Affected both `LeapModelDownloader` and `LeapSDK` SPM products; v0.10.7 retained the broken setup. (Public issue #5 reported v0.10.6+; the v0.10.5 affectedness rested on the project's internal `otool -L` evidence — the v0.10.9 release notes' v0.10.5–v0.10.8 span corroborates it.)
- **Evidence baselines (parked worktrees — keep only if the material-benefit trigger ever revives the migration):**
  - `~/AI-projects/arigato-ai-p2` — v0.10.6 attempt, branch `p2-leap-migration`, HEAD `d8e65d9` (5 checkpoints, builds clean, crashed at launch via `LeapModelDownloader.framework`'s inner dylib — pre-v0.10.9).
  - `~/AI-projects/arigato-ai-p2-v0.10.5` — v0.10.5 retry, branch `p2-v0.10.5-attempt`, HEAD `3b72378` (2 checkpoints, builds clean, crashed at launch via `LeapSDK.framework`'s inner dylib — LeapSDK transitively linked via LeapModelDownloader product; pre-v0.10.9).
- **GitHub issue:** https://github.com/Liquid4All/leap-sdk/issues/5 (OPEN but stale — fix shipped without closing it)
- **Trigger to revisit (supersedes the prior "monitor for a fix" trigger):** the upstream block is cleared, so the old monitor-for-a-fix trigger is **discharged.** Re-open this decision ONLY if a future leap-sdk release ships a capability that materially benefits THIS app (per the standing principle above). If/when that happens, the parked worktrees are the starting point — most behavioral changes (B1, B2, B3, B6, B9) were verified at both v0.10.5 and v0.10.6 via `.swiftinterface` grep — but re-verify against the then-current release before trusting them.
- **Cross-references:** V3 entry `b851dad` (LFM2 download — LEAP portal stale). `docs/PHASE_5_B1_1_MIGRATION_INVENTORY.md` §5 (P-2 Execution Results). Dependent entries below are now superseded by this decision: "leap-sdk v0.10.x migration path — re-evaluate B1.1 after issue #5 closes", "Adopt Dynamic Workflows for the v0.10.x LEAP migration", "Issue #5 escalation cadence" (CLOSED). Canonical live status: `docs/CURRENT_STATE.md` "Upstream status" section.
- **Severity:** LOW — upstream-resolved and decision-locked; no longer blocking and no open action. (Was HIGH while the block was live and B1.1 was a ship-blocker.)
...
### leap-sdk v0.10.x migration path — re-evaluate B1.1 after issue #5 closes — SUPERSEDED 2026-05-31 (decision: not adopting)

- **STATUS (2026-05-31): SUPERSEDED by the locked "NOT adopting v0.10.9" decision** (see "LEAP SDK v0.10.x migration — upstream fix shipped (v0.10.9), NOT adopting"). This entry's premise — "when issue #5 closes" — is moot: the @rpath fix shipped in v0.10.9 (2026-05-29) WITHOUT closing issue #5. The migration is now deferred **by decision** (no material benefit to a JA↔EN translator; SDK version ≠ translation quality), not by upstream block. The API-surface rewrite detail below is retained ONLY as reference for IF the standing material-benefit trigger ever fires.
...
- **Trigger to revisit:** **discharged** — superseded by the locked decision. Re-open ONLY under the standing material-benefit principle (a future leap-sdk release delivering a capability that materially touches THIS app). The "+30-day escalation cadence" path is **closed** — the upstream fix (v0.10.9) made escalation and the `install_name_tool` self-unblock moot.
- **Cross-references:** issue #5 (https://github.com/Liquid4All/leap-sdk/issues/5); "LEAP SDK v0.10.x migration — upstream fix shipped (v0.10.9), NOT adopting" (canonical record); "Issue #5 escalation cadence" (CLOSED); "Liquid Docs MCP — installed". No project memory on the LEAP SDK tag currently exists; create one pinning the channel reality (leap-ios v0.9.4 ship pin; leap-sdk v0.10.x is the live-but-unadopted channel) if the migration is ever revived.
- **Severity:** LOW — superseded 2026-05-31; never blocking after the 2026-05-25 B1.1 downgrade (MVP-1 ships on LEAP v0.9.4).
...
- **Trigger:** the v0.10.x migration is **NOT greenlit** as of 2026-05-31 (locked decision: not adopting v0.10.9 — no material benefit). The upstream block already cleared (v0.10.9 / 2026-05-29), so "when issue #5 unblocks" is satisfied but **insufficient on its own** — this tooling note fires ONLY if the standing material-benefit principle ever revives the migration. Independently, keep watching for Dynamic Workflows reaching GA.
- **Severity:** LOW (migration not adopted; would only revive on material benefit).

### Issue #5 escalation cadence — CLOSED 2026-05-31 (upstream fix shipped; escalation moot)

- **STATUS (2026-05-31): CLOSED.** The reason for an escalation cadence — an open-ended wait on an unengaged maintainer — no longer exists: the @rpath fix shipped upstream in **v0.10.9 (2026-05-29)**. No escalation (+14d / +30d Discord/social pings) is needed, and the `install_name_tool -change` self-unblock workaround is moot (the dylib is fixed at the source). Issue #5 is still OPEN only because the maintainer didn't close it; the project will not migrate regardless (locked decision: not adopting v0.10.9). **The two cadence dates (2026-06-03 / 2026-06-19) are discharged — do NOT action them.**
- **Historical (pre-resolution):** issue #5 was OPEN, filed by the project (OnigiriMerchant) 2026-05-20, with zero maintainer engagement as of 2026-05-25; the cadence existed to bound the open-ended wait while B1.1 was still treated as the last MVP-1 blocker (it was later downgraded 2026-05-25; MVP-1 ships on v0.9.4).
- **Cross-references:** issue #5; "LEAP SDK v0.10.x migration — upstream fix shipped (v0.10.9), NOT adopting".
