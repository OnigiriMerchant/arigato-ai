...
1. **Cheap path — `ENABLE_DEBUG_DYLIB = NO` on the host target.** Single-line build-settings change. Re-enables the simple TEST_HOST drop. Cost: SwiftUI Previews stop working on the executable target until views are moved to a framework/package target. For a project whose Phase 7 design-language work likely involves heavy Preview-driven iteration, this cost may be unacceptable. Worth measuring (how much Preview iteration is happening today?) before deciding.
