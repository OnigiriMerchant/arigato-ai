...
| **`.ended(meetingID, startedAt, endedAt)`** | `mm:ss` final duration (no `REC` prefix, no pulse), `BadgeKind.ended` glyph (system stop square) (`MeetingControlsView.swift:647-653, 206-210`). | `NEW TRANSCRIPT` (`MeetingControlsView.swift:672-673`) | None — `secondaryButton(for:.ended)` returns `nil` after B1.4 (commit `f018a71`). Share moved to the active-view toolbar per UI #9 Context A; see "Active-view toolbar surface" below. Gap #4 RESOLVED. | Split-screen still renders persisted sentences for `meetingID` because `activeMeetingID(in:)` returns the `meetingID` even for `.ended`. The split-screen VM stays mounted post-end per UI #5. `sentencesDidUpdate` callback is still installed but no new sentences arrive (pipeline torn down by `MeetingCoordinator.finalizeStop`, `MeetingCoordinator.swift:239-243`). | Row updates: `endedAt` now non-nil, duration column renders `"N min"` (`MeetingListRow.swift:90-95`). Title rewritten from `firstEnglishSentence` via `MeetingSession.finalizeStop` (`MeetingSession.swift:376-394`). | Same as `.recording`. |

## Active-view toolbar surface

Added 2026-05-21 to track the post-B1.4 active-view toolbar. The toolbar attaches to `ContentView`'s `NavigationStack.mainContent` and currently hosts two `ToolbarItem(placement: .topBarTrailing)` elements rendered left-to-right in declaration order: History (clock icon) then Share (square.and.arrow.up icon).

| Phase | Toolbar History icon | Toolbar Share icon |
|---|---|---|
| **`.idle`** | Visible when `bootstrapper.meetingStore != nil` (`ContentView.swift:138-149`) | Not present — `makeShareModel()` returns `nil` outside `.ended` (`ContentView.swift:215-228`). |
| **`.recording`** | Same as `.idle`. | Not present. |
| **`.paused`** | Same as `.idle`. | Not present. |
| **`.stoppingWithUndoWindow`** | Same as `.idle`. | Not present. |
| **`.ended(meetingID, _, _)`** AND `snapshot.sentences.isEmpty == false` | Same as `.idle`. | **Visible.** Renders `ShareLink(item: exportURL)` where `exportURL` is composed via the same `TranscriptExporter` pipeline as `MeetingDetailView.exportURL`. Tap raises the iOS share sheet with the transcript Markdown file attached. (`ContentView.swift:177-188`, `ActiveMeetingShareViewModel.swift`.) |
| **`.ended(meetingID, _, _)`** AND empty meeting | Same as `.idle`. | Not present — the snapshot's `sentences.isEmpty` check gates the toolbar item. (`ContentView.swift:177-180`.) |

Source-of-truth: `ContentView.swift` `makeShareModel()`, `activeShareIdentity()`, `activeMeetingExportURL(snapshot:)`. The VM-level atomic-snapshot contract is locked by `ActiveMeetingShareViewModelTests` (9 reload-contract tests).
...
### Gap #4 — Controls cluster `Share` placeholder vs MeetingDetailView functional ShareLink. **[RESOLVED 2026-05-21 by B1.4 commit `f018a71`]**

*Original finding (2026-05-17):* `MeetingControlsView.dispatchSecondary(.share)` was a no-op with the comment "Step 13 will wire a real `ShareLink`." The functional share was wired in `MeetingDetailView.swift:119-127` (Context B). In the `.ended` phase, the user saw a `Share` button on the controls cluster that did nothing.

*Resolution:* B1.4 implemented UI #9 Context A by removing the cluster-Share no-op (formatter `.share` case, `SecondaryKind.share`, and `dispatchSecondary(.share)` all deleted) and wiring a `ShareLink` to `ContentView`'s `.topBarTrailing` toolbar slot. The Share button now appears on the active view when `phase == .ended` AND the meeting has ≥1 sentence. See "Active-view toolbar surface" below for the new phase-gated visibility contract.

*Smoke caveat:* The load-bearing tap-raises-share-sheet behavior is unverified end-to-end because LFM2 is upstream-blocked (V3 `b851dad`). Static visibility + the unit-test contract lock (`ActiveMeetingShareViewModelTests`) are the achievable bars. Live tap verification deferred — see post-B1.4 follow-up "Live-tap verification trigger" in V3_BACKLOG.md.
