...
/// `any WhisperClient` reference is `Sendable` by protocol conformance,
/// so the enum is safe to ferry across actor boundaries.
///
/// Access level is `internal` because the associated payload of
/// ``loaded(_:)`` is the internal ``WhisperClient`` protocol; this type
/// cannot be `public` without first promoting ``WhisperClient`` to
...
nonisolated enum LoaderState {
    ...
    /// The model is loaded and the wrapped client is ready for inference.
    case loaded(any WhisperClient)
...
/// Owns the lifecycle of the underlying Whisper client and coalesces
...
/// and the Whisper-backed client. It is an actor so the underlying
/// client — which is not `Sendable` in WhisperKit v1.0.0 — can be held
/// safely across async boundaries.
///
/// Only one client instance exists per loader at a time. Variant choice
...
/// client without a reload (see ``loadIfNeeded(variant:)`` for details).
///
/// The actor and its API surface are `internal`. ``loadIfNeeded(variant:)``
/// returns `any WhisperClient` (an internal protocol) and ``currentState()``
...
actor WhisperModelLoader {
    ...
    private var inFlight: Task<any WhisperClient, Error>?
    private let factory: WhisperEngineFactory

    /// Production initializer. Wires the loader to the real WhisperKit
    /// factory at ``WhisperClientFactory/make``.
    init() {
        factory = WhisperClientFactory.make
    }
    ...
    /// Loads the requested ``WhisperModelVariant`` if no client is already
    /// loaded; coalesces concurrent calls into a single in-flight task.
    ///
    /// Behaviour:
    /// - If the loader is already in ``LoaderState/loaded(_:)``, the
    ///   wrapped client is returned immediately. **Variant mismatch is
    ///   silently ignored**: callers requesting a different variant after
    ///   one is loaded receive the already-loaded client. This matches
    ///   the single-client ownership invariant; phase-4 ships only one
    ...
    ///   ``state`` becomes ``LoaderState/loaded(_:)`` and the client is
    ...
    func loadIfNeeded(
        ...
        let task = Task<any WhisperClient, Error> { [factory] in
    ...
    /// Resets the loader to ``LoaderState/idle``, releasing the client
